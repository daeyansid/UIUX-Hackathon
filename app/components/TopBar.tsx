            ...
            <div className="bg-bgFillColor">
                <div className="container mx-auto px-4 sm:px-8 lg:px-16 xl:px-20 max-w-7xl">
                    <div className="py-3 flex flex-col sm:flex-row items-center justify-between gap-4">
                        ...
                        {/* Cart Button */}
                        <Link href="/checkout">
                            <p className="inline-flex items-center justify-center rounded-md border border-white bg-white w-full sm:w-28 h-10 px-3 py-2 font-medium text-textColor shadow-1 hover:bg-gray-100 transition-colors">
                                <span className="mr-2">
                                    <Image src="/cart.svg" alt="cart" width={20} height={20} />
                                </span>
                                Cart
                                <span className="ml-2">
                                    <Image src="/cart-num.svg" alt="cart" width={20} height={20} />
                                </span>
                            </p>
                        </Link>
                    </div>
                        ...
                        <ul className="h-full lg:flex lg:items-center lg:gap-x-8 max-lg:space-y-3 max-lg:fixed max-lg:bg-white max-lg:w-1/2 max-lg:min-w-[300px] max-lg:top-0 max-lg:left-0 max-lg:p-6 max-lg:h-full max-lg:shadow-md max-lg:overflow-auto z-50">
                            ...
                            {menuItems.map((item, index) => (
                                <li key={index} className="max-lg:border-b max-lg:py-3">
                                    <Link href={item.path}>
                                        <p
                                            className={`hover:text-[#007bff] ${item.name === 'Home' ? 'text-[#007bff]' : 'text-gray-600'
